import { FiX, FiFileText, FiDownload, FiMessageCircle, FiMapPin, FiBriefcase, FiCalendar, FiUser, FiMail, FiPhone } from "react-icons/fi";
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
            ...
            {application.resumeUrl ? (
              <a
                href={getFullFileUrl(application.resumeUrl)}
                target="_blank"
                rel="noopener noreferrer"
                download={application.resumeName || "Resume.pdf"}
                className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3 text-xs font-bold text-brand shadow-xs transition hover:border-brand dark:border-slate-700 dark:bg-slate-900"
              >
                <div className="flex items-center gap-2">
                  <FiFileText className="text-base" />
                  <span className="truncate max-w-[150px]">{application.resumeName || "View Resume"}</span>
                </div>
                <FiDownload />
              </a>
            ) : (
